...
SpectralDistribution::SpectralDistribution(const std::vector<Float>& lambdas, const std::vector<Float>& vals) {
    _num = vals.size();
    if (vals.size() != lambdas.size()) {
        // Error

        _num = std::min(vals.size(), lambdas.size());
    }

    // Sort both vectors in increasing lambda
    /*auto permutation = Utils::sortPermutation(lambdas, _num, std::less<Float>());
    _lambdas = Utils::applyPermutation(lambdas, permutation);
    _vals    = Utils::applyPermutation(vals, permutation);*/
}

SpectralDistribution::SpectralDistribution(const std::string& filename) {
    std::vector<float> lambdas, vals;
    //Utils::readFunction1D(filename, lambdas, vals);

    SpectralDistribution(lambdas, vals);
}

float SpectralDistribution::linearInterpolate(float lambda) const {
    // Find upper bound index
    auto it = std::upper_bound(_lambdas.begin(), _lambdas.end(), lambda);
    uint32 idx = std::distance(_lambdas.begin(), it) - 1;

    // Find interpolation weight
    double t = (lambda - _lambdas[idx]) 
             / (_lambdas[idx + 1] - _lambdas[idx]);

    return math::lerp(t, _vals[idx], _vals[idx + 1]);
}

RGBSpectrum SpectralDistribution::toRGB() const {
    RGBSpectrum xyz = RGBSpectrum(0);

    // 1. Integrate the spectral distribution with the XYZ basis
    // - Integration performed with Riemmann sums
    for (uint32 k = 0; k < CIENumSamples; ++k) {
        double val = linearInterpolate(CIELambdas[k]);

        xyz.r += val * CIESamplesX[k];
        xyz.g += val * CIESamplesY[k];
        xyz.b += val * CIESamplesZ[k];
    }

    xyz *= (CIELambdas[CIENumSamples - 1] - CIELambdas[0])
        / (CIEYIntegral * CIENumSamples);

    // 2. Convert from XYZ to linear RGB
    return XYZToRGB(xyz);
}

extern const double pbr::CIELambdas[] = {
    360, 361
};

extern const double pbr::CIESamplesX[] = {
...
extern const double pbr::CIESamplesY[] = {
...
extern const double pbr::CIESamplesZ[] = {
